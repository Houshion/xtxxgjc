...
var baseUrl = "http://xxgjc.app.xiaozhuschool.com";
var payType = checkBrowser();
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code == 101) {
                ...
                setTimeout(() => {
                    checkLogin()
                    // location.href = "../html/binding.html"
                }, 1500);
            } else {
                callback(data);
            }
    ...
    WeixinJSBridge.invoke(
        ...
        function (res) {
            ...
            if (res.err_msg == "get_brand_wcpay_request:ok") {
                toast("支付成功");
                setTimeout(() => {
                    url ? window.location.replace(url) : close.wx();
                }, 1500);
            } else {
                toast("取消支付");
            }
...
(function (D) {
    function p(b, e, c) {
        ...
        this.getHMAC = function (c, b, e, g, q) {
            ...
            for (l = 0; l <= b; l += 1)
                (p[l] = h[l] ^ 909522486), (t[l] = h[l] ^ 1549556828);
        ...
        if ("UTF8" === e)
            for (g = 0; g < b.length; g += 1)
                for (
                    ...
                    d.push(a) :
                    2048 > a ?
                    (d.push(192 | (a >>> 6)), d.push(128 | (a & 63))) :
                    55296 > a || 57344 <= a ?
                    d.push(224 | (a >>> 12), 128 | ((a >>> 6) & 63), 128 | (a & 63)) :
                    ((g += 1),
    ...
    function A(b, e) {
        ...
        for (d = 0; d < a; d += 3)
            for (
                g =
                (((b[d >>> 2] >>> (8 * (3 - (d % 4)))) & 255) << 16) |
                (((b[(d + 1) >>> 2] >>> (8 * (3 - ((d + 1) % 4)))) & 255) << 8) |
                ((b[(d + 2) >>> 2] >>> (8 * (3 - ((d + 2) % 4)))) & 255),
                f = 0; 4 > f; f += 1
            )
                c =
                8 * d + 6 * f <= 32 * b.length ?
    ...
    function r(b, e) {
        ...
        return (
            ((((b >>> 16) + (e >>> 16) + (c >>> 16)) & 65535) << 16) | (c & 65535)
        );
    }
        ...
        for (u = 0; u < s; u += 16) {
            ...
            for (k = 0; 80 > k; k += 1)
                (c[k] =
                    16 > k ?
                    b[k + u] :
                    q(c[k - 3] ^ c[k - 8] ^ c[k - 14] ^ c[k - 16], 1)),
                (p =
                    20 > k ?
                    t(q(a, 5), (d & f) ^ (~d & g), m, 1518500249, c[k]) :
                    40 > k ?
                    t(q(a, 5), d ^ f ^ g, m, 1859775393, c[k]) :
                    60 > k ?
                    t(q(a, 5), (d & f) ^ (d & g) ^ (f & g), m, 2400959708, c[k]) :
                    t(q(a, 5), d ^ f ^ g, m, 3395469782, c[k])),
                (m = g),
                (g = f),
                (f = q(d, 30)),
                (d = a),
    ...
    var mySwiper = new Swiper(".touchLoad", {
        ...
        on: {
            ...
            touchEnd: function (swiper) {
                var _viewHeight = document.getElementsByClassName("swiper-wrapper")[
                    index
                ].offsetHeight;
                var _contentHeight = document.getElementsByClassName("swiper-slide")[
                    index
                ].offsetHeight;
                // 上拉加载
                if (
...
function toast(content, type) {
    return $.toast(content, type ? type : "text");
}
...
function checkLogin(callback) {
    if (checkBrowser() == 1) {
        // 当前微信浏览器
        if (!getItem("WxUserInfo") && !getParam().code) {
            setItem("mac_no", UrlSearch("mac_no"))
            setItem("push_id", UrlSearch("push_id"))
            window.location.replace(
                "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx33442761d591e554&redirect_uri=" +
                window.location.href +
                "&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect"
            );
        } else if (getItem("WxUserInfo")) {
            callback();
        } else {
            ajaxPost({
                    api_name: "weixinLogin",
                    code: getParam().code
                },
                "/App/home/index",
                function (res) {
                    if (res.code == 1) {
                        setItem("WxUserInfo", JSON.stringify(res.data));
                        ajaxPost({
                            ...
                            function (res1) {
                                if (res1.code == 1) {
                                    res.data.age = res1.data.age
                                    res.data.phone = res1.data.mobile
                                    setItem("WxUserInfo", JSON.stringify(res.data));
                                    callback();
                                }
                            }
        ...
        if (!getItem("AliUserInfo")) {
            window.location.replace(
                "https://openauth.alipaydev.com/oauth2/publicAppAuthorize.htm?app_id=2018120362429007&scope=auth_base&redirect_uri=" +
                window.location.href
            );
        }
    }
